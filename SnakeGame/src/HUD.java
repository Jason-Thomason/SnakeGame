...
public class HUD {
	...
	public void render(Graphics g) {
		...
		for (int i = 0; i < World.players; i++) {
			g.setFont(font);
			g.setColor(World.colors[i]);
			if (World.snakes.get(i).lives > 0) {
				g.drawString("" + World.snakes.get(i).lives, (Window.width / 2)
						+ (40 * ((int)Math.ceil((double)i/2)) * ((int) Math.pow(-1, i))), 50);
			}
			if (World.snakes.get(i).lives == 0 && World.snakes.get(i).dead) {
				g.drawString(
						"Dead",
						(Window.width / 2) + (40 * ((int)Math.ceil((double)i/2)) * ((int) Math.pow(-1, i))) - 20,
						50);
			}
			if (World.snakes.get(i).dead && World.snakes.get(i).lives > 0) {
				g.drawString(
						"" + (5 - (World.snakes.get(i).deathTimer / 10)),
						(Window.width / 2) + (40 * ((int)Math.ceil((double)i/2)) * ((int) Math.pow(-1, i))),
						70);
			}
		}
	}
