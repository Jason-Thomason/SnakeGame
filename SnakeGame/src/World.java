...
public class World {
	...
	public void tick() {
		...
		if (fruits.size() < Math.floor(Core.timer / 40) + 1) {
			newFruit();
		}
		if (fruits.size() > Math.floor(Core.timer / 40) + 1) {
			removeFruit();
	...
	public void checkCollisions() {
		for (int s = 0; s < snakes.size(); s++) {
			// Check obstacles
			if (snakes.get(s).snakeParts.size() > 0) {
				for (Obstacle o : obstacles) {
					if (snakes.get(s).size > 0) {
						o.checkCollisions(snakes.get(s).snakeParts.get(0)
								.getX(), snakes.get(s).snakeParts.get(0).getY());
					}
				}
				// Check fruit
				for (int i = 0; i < fruits.size(); i++) {
					if (snakes.get(s).size > 0) {
						if (snakes.get(s).snakeParts.get(0).getX() == fruits
								.get(i).x
								...
								if (snakes.get(s).size > 0) {
									if (snakes.get(s).snakeParts.get(0).getX() == snakes
										...
										if (snakes.get(s).size >= snakes.get(q).size
											...
											// Is snake s is bigger than snake
											// q's
	...
	public void nextLevel() {
		...
		snakes.add(new Snake(Color.BLUE, MapHandler.snake1StartingX,
				MapHandler.snake1StartingY));
		if (players == 2) {
			snakes.add(new Snake(Color.RED, MapHandler.snake2StartingX,
					MapHandler.snake2StartingY));
		}
		newFruit();
	}
